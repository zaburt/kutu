class GameCategoriesController < ApplicationController
  ...
  def create
    ...
    if @game_category.save
      redirect_to @game_category, notice: t('generic.was_successfully_created',
                                            :name => GameCategory.model_name.human)
    else
      render :new
    end
  end
  ...
  def update
    if @game_category.update(game_category_params)
      redirect_to @game_category, notice: t('generic.was_successfully_updated',
                                            :name => GameCategory.model_name.human)
    else
      render :edit
    end
  ...
  def destroy
    ...
    redirect_to game_categories_url, notice: t('generic.was_successfully_destroyed',
                                               :name => GameCategory.model_name.human)
  end
